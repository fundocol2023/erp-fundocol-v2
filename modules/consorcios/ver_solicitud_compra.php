...
// ✅ Solo roles permitidos para esta vista
if (!in_array($rol, [4, 8, 13])) {
    echo "<div class='alert alert-danger'>Sin permisos para acceder a esta solicitud.</div>";
    exit();
}
...
// ✅ Regla especial rol 13:
// solo puede ver solicitudes del Consorcio Aburra 2025
if ($rol == 13 && trim($solicitud['consorcio']) !== 'Consorcio Aburra 2025') {
    echo "<div class='alert alert-danger'>Sin permisos para acceder a esta solicitud.</div>";
    exit();
}

// Base URL para cotizaciones
$cotizacion_base = '../../modules/consorcios/uploads/cotizaciones/';

// 2. Traer productos
$sql = "SELECT * FROM solicitudes_compra_consorcios_productos WHERE solicitud_compra_id = ?";
$stmt = $pdo->prepare($sql);
$stmt->execute([$id]);
...
$subtitulo_revision = "Revisión y aprobación de compra de consorcios";
if (in_array($rol, [4, 13])) {
    $subtitulo_revision = "Revisión y aprobación de compra de consorcios (Presupuesto)";
} elseif ($rol == 8) {
    $subtitulo_revision = "Revisión y aprobación de compra de consorcios (Contabilidad)";
}
?>
<!DOCTYPE html>
<html lang="es">
...
<body class="consorcios-ver-solicitud-page">
...
<div class="erp-card">
    ...
    <div class="solicitud-header">
        ...
        <div class="solicitud-subtitle"><?= htmlspecialchars($subtitulo_revision) ?></div>
    </div>
