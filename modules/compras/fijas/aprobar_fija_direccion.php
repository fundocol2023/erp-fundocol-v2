...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($accion === 'aprobar') {
        try {
            ...
            // Direccion aprueba -> pasa a Contabilidad
            $pdo->prepare("
            ...
            $correo_contabilidad = "analista.contable@fundocol.org";
            $nombre_contabilidad = "Contabilidad Fundocol";
            $asunto = "Compra Fija pendiente de aprobacion (Contabilidad)";
            ...
            if (!empty($solicitud['observaciones_presupuesto'])) {
                $bloque_obs_presupuesto = "
                <div style='background:#eef6ff; padding:12px 15px; border-radius:10px; margin-bottom:15px; border-left:4px solid #0ea5e9;'>
            ...
            if (!empty($observaciones)) {
                $bloque_obs_direccion = "
                <div style='background:#fff7e6; padding:12px 15px; border-radius:10px; margin-bottom:15px; border-left:4px solid #f59e0b;'>
                    <p style='margin:0;'><b>Observaciones de Dirección:</b><br>" . nl2br(htmlspecialchars($observaciones)) . "</p>
                </div>
                ";
            }
...
<div style='font-family:Arial, sans-serif; color:#1f2937; line-height:1.6; max-width:600px;'>
    ...
    <h2 style='color:#0ea5e9; margin-bottom:15px;'>
        Nueva Compra Fija para aprobar en Contabilidad
    </h2>
    ...
    <div style='text-align:center; margin:30px 0;'>
        <a href='{$link}'
           style='background:#0ea5e9; color:#ffffff; padding:14px 32px;
            ...
            Revisar en ERP Fundocol
        </a>
    </div>

    <p style='font-size:13px; color:#6b7280; text-align:center;'>
        Mensaje automatico ERP Fundocol
    </p>
        ...
        if (empty($observaciones)) {
            ...
            if (!empty($solicitud['solicitante_email'])) {
                ...
                $mensaje_html = "
                    ...
                    <p>Tu solicitud fue rechazada por Direccion.</p>
...
<head>
    <meta charset="UTF-8">
    <title>Revisión de Compra Fija (Dirección)</title>
...
<body class="compras-fijas-aprobar-direccion-page">
<div class="navbar-spacer"></div>
<div class="aprobacion-box">
    <div class="aprob-title"><i class="bi bi-file-earmark-check"></i> Revisión de Compra Fija (Dirección)</div>
    ...
    <div class="aprob-section">
        ...
        <div class="aprobacion-grid">
            ...
            <?php if (!empty($solicitud['observaciones'])): ?>
                <div class="label-erp">Observaciones del solicitante:<span class="valor-erp"><?= htmlspecialchars($solicitud['observaciones']) ?></span></div>
            <?php endif; ?>
